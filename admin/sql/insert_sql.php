...
if (isset($_POST['signup'])) 
{
	...
	if ($conn->query($login) === TRUE) 
	{
			...
			$reg = "INSERT INTO registration(name,email,department,member_id,login_id,created_at) VALUES ('$name','$email','$department','$member_id',$last_id,now())";
...
elseif(isset($_POST['assign_teacher'])) 
{
	...
	// var_dump($_POST);
	// exit;
	$validation = "SELECT * FROM assign_teacher where session_id=$session and teacher_id='$teacher'";

	$result = $conn->query($validation);

	if($result->num_rows == 0){

		$sql = "INSERT INTO assign_teacher(teacher_id,session_id,subject_id,created_at) VALUES ('$teacher','$session','$subject',now())";
...
if ($conn->query($login) === TRUE) 
{
		$last_id = $conn->insert_id;
		//FOR REGISTRATION FORM//
		$reg = "INSERT INTO registration(name,email,department,member_id,login_id,created_at,authentication) VALUES ('$name','$email','$department','$student_id',$last_id,now(),1)";
